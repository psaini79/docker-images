...
if [ "${ORACLE_SID}" = "FREE" ]; then
  ...
  if [ "${TRUE_CACHE}" == "true" ]; then
    ...
    dbca -silent -createTrueCacheInstance -dbUniqueName "$TRUEDB_UNIQUE_NAME" -gdbName "$ORACLE_SID" -sid "$ORACLE_SID" -sourceDBConnectionString "$PRIMARY_DB_CONN_STR" $SOURCE_DB_BASED_ARGS  ORACLE_HOSTNAME="$ORACLE_HOSTNAME" <<EOF
${ORACLE_PWD}
EOF
...
if [[ "${CLONE_DB}" == "true" ]] || [[ "${STANDBY_DB}" == "true" ]] || [[ "${TRUE_CACHE}" == "true" ]]; then
  ...
  if [ "${STANDBY_DB}" = "true" ]; then
      ...
      if [ -n "$TRUE_CACHE_BLOB" ]; then
	    ...
	    dbca -silent -createTrueCacheInstance -dbUniqueName "$TRUEDB_UNIQUE_NAME" -gdbName "$ORACLE_SID" -sid "$ORACLE_SID" -sourceDBConnectionString "$PRIMARY_DB_CONN_STR" ${DBCA_CRED_OPTIONS} $SOURCE_DB_BASED_ARGS $SGA_TARGET_IN_MB $PGA_AGGREGATE_TARGET_IN_MB ORACLE_HOSTNAME="$ORACLE_HOSTNAME" <<EOF
${ORACLE_PWD}
${TDE_WALLET_PWD}
EOF
	[ $? -eq 0 ] || cat /opt/oracle/cfgtoollogs/dbca/"$ORACLE_SID"/"$ORACLE_SID".log || cat /opt/oracle/cfgtoollogs/dbca/"$ORACLE_SID".log
