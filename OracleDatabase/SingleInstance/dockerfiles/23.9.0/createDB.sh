...
if [ "${ORACLE_SID}" = "FREE" ]; then
  ...
  if [ "${TRUE_CACHE}" == "true" ]; then
    ...
    dbca -silent -createTrueCacheInstance -dbUniqueName "$ORACLE_SID"_TC -gdbName "$ORACLE_SID" -sid "$ORACLE_SID" -sourceDBConnectionString "$PRIMARY_DB_CONN_STR" $SOURCE_DB_BASED_ARGS  ORACLE_HOSTNAME="$ORACLE_HOSTNAME" <<EOF
${ORACLE_PWD}
EOF
  ...
  /etc/init.d/oracle-free-23ai configure << EOF
${ORACLE_PWD}
${ORACLE_PWD}
EOF
